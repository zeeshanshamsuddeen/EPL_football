<?php 
   Class Main_model extends CI_Model { 
    	...
    	public function get_next_match(){
	      	$sql= "SELECT * FROM fixtures WHERE played=0 ORDER BY game_id LIMIT 1";
     	...
     	public function get_matches_3(){
	      	$sql= "SELECT * FROM fixtures WHERE played=0 ORDER BY game_id LIMIT 3";
	   		$data=$this->db->query($sql);
	   		return $data->result();
     	}

		public function get_team_details(){
			$sql= "SELECT * FROM teams ORDER BY POINTS desc,GD desc";
			$data = $this->db->query($sql);
			return $data->result();
		}  
		...
		public function get_fixtures(){
			$sql = "SELECT * FROM fixtures WHERE played=0 order by game_id ";
			$data = $this->db->query($sql);
			return $data->result();
		}
		...
		public function get_games_full(){
		...
		public function get_desc_player() { 
			$sql="SELECT *  from players order by price desc,name desc";
			$row= $this->db->query($sql);
			return $row->result(); 
		}
		...
		public function get_players_admin(){
     	...
     	public function get_games_players_full_group_id_goals(){
     		$sql = "SELECT games.player_id,players.NAME,players.TEAM,SUM(games.goals) AS sum FROM games INNER JOIN players ON games.player_id = players.ID GROUP BY games.player_id ORDER BY SUM(games.goals) DESC LIMIT 5 ";
     		$data = $this->db->query($sql);
			return $data->result();
     	}

     	public function get_games_players_full_group_id_assists(){
     		$sql = "SELECT games.player_id,players.NAME,players.TEAM,SUM(games.assists) AS sum FROM games INNER JOIN players ON games.player_id = players.ID GROUP BY games.player_id ORDER BY SUM(games.assists) DESC LIMIT 5 ";
     		$data = $this->db->query($sql);
			return $data->result();
     	}
     	

		public function get_games_players_full_group_id_yellow(){
     		$sql = "SELECT games.player_id,players.NAME,players.TEAM,SUM(games.yellow) AS sum FROM games INNER JOIN players ON games.player_id = players.ID GROUP BY games.player_id ORDER BY SUM(games.yellow) DESC LIMIT 5 ";
     		$data = $this->db->query($sql);
			return $data->result();
     	}
		...
		public function put_match_details($data){
			...
			{
					$sql = "UPDATE teams SET DRAW=DRAW+1 WHERE NAME='{$fixtures['play_team_2']}'";
					$this->db->query($sql);
					$sql = "UPDATE teams SET DRAW=DRAW+1 WHERE NAME='{$fixtures['play_team_1']}'";
					$this->db->query($sql);
